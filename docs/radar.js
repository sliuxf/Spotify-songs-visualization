...
function RadarChart(id, data, options) {
	var cfg = {
	 ...
	 opacityArea: 0.44, 	//The opacity of the area of the blob
	...
	var allAxis = (data[0].map(function(i, j){return i.axis})),	//Names of each axis
		...
		Format = d3.format('.4f'),			 	//Percentage formatting
	...
	axis.append("text")
		...
		//.attr("dy", "0.35em")
		.attr("x", function(d, i){ return 1.2*rScale(maxValue * cfg.labelFactor) * Math.cos(angleSlice*i - Math.PI/2); })
		.attr("y", function(d, i){ return 1.05*rScale(maxValue * cfg.labelFactor) * Math.sin(angleSlice*i - Math.PI/2); })
	...
	var LegendOptions = cfg.tracklist;
	LegendOptions = LegendOptions.map(function(x){ return x.toUpperCase() })

	var code = ["#8a3ebe"];
	var code2 = ["#1662ca","#f5c71a"];

	if(LegendOptions.length == 2){
		code = code2;
	}
    ...
    svg.selectAll("dots").data(code)
	   .enter().append("circle")
	   .attr("cx", width+25)
	   .attr("cy", function(d,i){ return i*25+85}) 
	   .attr("r", 7)
